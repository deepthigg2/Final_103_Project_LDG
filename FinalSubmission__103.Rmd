...
summary_table <- Genes_CoVariate_Data_3_3  %>%
  ...
  summarise(
    ...
    Median_hospital_free_days= median(`hospital-free_days_post_45_day_followup`,na.rm=TRUE),
    IQR_hospital_free_days = IQR(`hospital-free_days_post_45_day_followup`, na.rm = TRUE),
    Median_ventilator_free_days= median(`ventilator-free_days`, na.rm = TRUE),
    IQR_ventilator_free_days = IQR(`ventilator-free_days`, na.rm = TRUE),
    ICU_Status_Yes = round((sum(icu_status == "yes", na.rm = TRUE)/rows)*100,2), 
    ICU_Status_No = round((sum(icu_status == "no", na.rm = TRUE) /rows)*100,2),
        COVID_Disease_Status = round((sum(disease_status == "disease state: COVID-19", na.rm = TRUE)/rows)*100,2),
    Non_COVID_Disease_Status  = round((sum(disease_status == "disease state: non-COVID-19", na.rm = TRUE) /rows)*100,2)
    )

Summary_table_t <- as.data.frame(t(summary_table))
names(Summary_table_t) <- Summary_table_t[1,]
Summary_table_t <- Summary_table_t[-1,]

tab <- kable(x = Summary_table_t, caption = 'Summary Table',
             format = 'latex',booktabs = T,
             col.names = c("Female", "Male","Unknown" ),
